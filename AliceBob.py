...
'''
The intercept function takes the classical encoded
message, along with a new qubit not from the Bell pair. 
Then Pauli-X and/or Pauli-Z gates are applied to the Qubit,
conditionally on the values in the message. This is to test if an unentangled 
qubit can be used to decode the message, if it were intercepted by a third party
'''
def message_interceptor(quantum_engine, message):
    '''
    Pauli-X and/or Pauli-Z gates are applied to the Qubit,
    conditionally on the values in the message.
    '''
    qubit_three = quantum_engine.allocate_qubit()
    if message[1] == 1:
        X | qubit_three
    if message[0] == 1:
        Z | qubit_three
...
def send_receive(bit=0,quantum_engine=''):
    ...
    if intercept:
        return message_interceptor(quantum_engine, classical_encoded_message)
    else:
...
def send_full_message(message='DataEspresso.com',quantum_engine=''):
    ...
    if intercept:
        print('Message has been intercepted by a third party, they have tried to decode it using a new Qubit')
...
quantum_engine_one=MainEngine()
message = 'Hello Bob its Alice'
send_full_message(message=message,quantum_engine=quantum_engine )
intercept = True
send_full_message(message=message,quantum_engine=quantum_engine_one )